...
GO
 ...
 CREATE OR ALTER PROCEDURE [dbo].[sp_Accounts_GetBillingDetails_Ver3_CurrentProd202311]
  @UserID INT,
          ...
          CurrencyConverterRateGroupID,
            ...
            ii.CurrencyConverterRateGroupID as CurrencyConverterRateGroupID,
                ...
                WHERE   i.UserID = @UserID
                        AND (ii.DateCreated >= @StartDate OR @StartDate IS NULL)
                        AND (ii.DateCreated < @EndDate OR @EndDate IS NULL)
        ...
        FROM    ( SELECT    t.InvoiceItemID,
              ...
              NULL as TransactionCodeActive
                  ...
                  WHERE     ubl.UserID = @UserID
                            AND (ubl.DateCreated >= @StartDate OR @StartDate IS NULL)
                            AND (ubl.DateCreated < @EndDate OR @EndDate IS NULL)
                            AND ubl.Amount < 0
              ...
              NULL as TransactionCodeActive
                  ...
                  WHERE     p.UserID = @UserID
                            AND (p.DateCreated >= @StartDate OR @StartDate IS NULL)
                            AND (p.DateCreated < @EndDate OR @EndDate IS NULL)
              ...
              INNER JOIN Financial.UserBalances ubl on ubl.LogID=uba.SourceLogID
                  ...
                  WHERE     uba.UserID = @UserID
                            AND (uba.DateCreated >= @StartDate OR @StartDate IS NULL)
                            AND (uba.DateCreated < @EndDate OR @EndDate IS NULL)
              ...
              NULL as TransactionCodeActive
                  ...
                  WHERE     pay.UserID = @UserID
                            ...
                            AND pay.typeID = 1
                            AND (ref.DateCreated >= @StartDate OR @StartDate IS NULL)
                            AND (ref.DateCreated < @EndDate OR @EndDate IS NULL)
                            AND ref.Status = 1
                  UNION
                  SELECT    0 AS InvoiceItemID,
                            0 AS PaymentID,
          ...
          SELECT    0 AS InvoiceItemID,
              ...
              INNER JOIN Financial.TransactionCodes tc on tci.TransactionCodeID = tc.TransactionCodeID
                  WHERE     w.UserID = @UserID
                            AND (w.DateCreated >= @StartDate OR @StartDate IS NULL)
                            AND (w.DateCreated < @EndDate OR @EndDate IS NULL)
